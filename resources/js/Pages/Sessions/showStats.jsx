        ...
        >
            ...
            <div className="py-12 bg-slate-950 min-h-screen text-slate-100">
                <div className="mx-auto max-w-7xl sm:px-6 lg:px-8 space-y-6">
                    ...
                    {/* Tarjeta de Resumen */}
                    <div className="bg-slate-900 overflow-hidden shadow-xl sm:rounded-2xl border border-slate-800 transition-all hover:shadow-2xl duration-300">
                        <div className="p-8 bg-gradient-to-br from-sky-600 to-slate-900 text-white relative overflow-hidden">
                            {/* Decorative background shapes */}
                            <div className="absolute -top-24 -right-24 w-64 h-64 bg-white opacity-10 rounded-full blur-2xl"></div>
                            <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-sky-400 opacity-20 rounded-full blur-2xl"></div>

                            <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
                                <div>
                                    <h3 className="text-3xl font-extrabold tracking-tight mb-2">Resumen del Juego</h3>
                                    <p className="text-sky-100 text-lg">Jugador: <span className="font-semibold text-white">{user?.name || 'Desconocido'}</span></p>
                                ...
                                <div className="text-right bg-white/10 backdrop-blur-md rounded-xl p-6 border border-white/20 shadow-inner">
                                    <p className="text-sm uppercase tracking-wider text-sky-100 font-semibold mb-1">Emoción Predominante</p>
                            ...
                            {!stats ? (
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                                        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-sm flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                                            <div>
                                                <p className="text-sm font-medium text-slate-400 mb-1">Total de Lecturas</p>
                                                <p className="text-3xl font-bold text-white">{stats.totalEmotions}</p>
                                            </div>
                                            <div className="h-12 w-12 rounded-full bg-sky-900/50 flex items-center justify-center text-sky-400">
                                        ...
                                        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-sm flex items-center justify-between hover:-translate-y-1 transition-transform duration-300">
                                            <div>
                                                <p className="text-sm font-medium text-slate-400 mb-1">Confianza Media</p>
                                    ...
                                    <div className="mt-8">
                                        <h4 className="text-xl font-bold text-white mb-6 border-b pb-2 border-slate-800">Distribución de Emociones</h4>
                                                    ...
                                                    return (
                                                        <div key={emotion} className="relative group">
                                                            <div className="flex justify-between items-center mb-1">
                                                                <span className="text-sm font-semibold text-slate-300 capitalize group-hover:text-sky-400 transition-colors">{emotion}</span>
                                                                <span className="text-sm text-slate-500 font-medium">{count} ({percentage}%)</span>
                                                            </div>
                                                            <div className="w-full bg-slate-800 rounded-full h-3.5 mb-1 overflow-hidden shadow-inner">
                                                                <div
                    ...
                    {stats && emotions.length > 0 && (
                        <div className="bg-slate-900 overflow-hidden shadow-lg sm:rounded-2xl border border-slate-800 p-8">
                            <h4 className="text-xl font-bold text-white mb-6">Registro Crudo (Últimas detecciones)</h4>
                            <div className="bg-slate-800 rounded-lg p-4 border border-slate-700 overflow-x-auto">
                                <table className="min-w-full text-left text-sm text-slate-300">
                                    ...
                                    <tbody>
                                        {emotions.slice(-10).reverse().map((emo, idx) => (
                                            <tr key={idx} className="border-b border-slate-700 last:border-0 hover:bg-slate-700 transition-colors">
                                                <td className="px-4 py-2">{emo.game_time ? `${emo.game_time}s` : `Id: ${emo.id}`}</td>
                                                <td className="px-4 py-2 capitalize font-medium text-white">{emo.emotion}</td>
                                ...
                                {emotions.length > 10 && (
                                    <p className="text-xs text-slate-500 mt-3 text-center italic">Mostrando las últimas 10 emociones capturadas.</p>
                                )}
                            </div>
                        </div>
                    )}
